...
export default async function IndustryNewsPage({ params }: { params: Promise<{ locale: string }> }) {
  const p = await params;
  const locale = resolveLocale(p.locale);
  const articles = await articleService.getAllSummary();
  const list = Array.isArray(articles) ? localizeAll(articles.filter((a: ArticleSummary) => a.category === 'tin-nganh' && !a.isDraft), locale) : [];
  ...
  return (
    <div className="bg-white min-h-screen">
      <PageHero
        locale={locale}
        align="left"
        eyebrow={locale === 'en' ? 'Market & trends' : 'Thị trường & xu hướng'}
        title={locale === 'en' ? 'Animal husbandry industry news' : 'Tin tức ngành chăn nuôi'}
        subtitle={locale === 'en' ? 'The latest news, trends and information on the veterinary market in Vietnam and worldwide.' : 'Cập nhật tin tức, xu hướng và thông tin mới nhất về thị trường thú y Việt Nam và Thế giới.'}
        breadcrumb={[
          { label: locale === 'en' ? 'News' : 'Tin tức', href: '/tin-tuc' },
          { label: locale === 'en' ? 'Industry news' : 'Tin tức ngành chăn nuôi' },
        ]}
      />
      ...
      <div className="container mx-auto px-4 py-16 lg:py-24">
        ...
        {list.length === 0 && (
          <div className="text-center py-20 bg-sand rounded-2xl border border-line">
             <span className="w-14 h-14 rounded-xl bg-primary/8 text-primary flex items-center justify-center mx-auto mb-5"><Globe size={26} /></span>
             <p className="text-ink-soft font-montserrat font-semibold uppercase tracking-[0.16em]">{locale === 'en' ? 'Updating new articles...' : 'Đang cập nhật tin tức mới...'}</p>
          </div>
        )}
      </div>
    </div>
